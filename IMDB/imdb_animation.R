...
#count total pages
url <- "http://www.imdb.com/search/title?genres=animation&title_type=feature&sort=moviemeter,asc"
res <- GET(url)
...
all_url <- sprintf("http://www.imdb.com/search/title?genres=animation&sort=moviemeter,asc&start=%s&title_type=feature",allpage)
title <- list()
year <- list()
...
colnames(data_final_2) <- c("name","year","genre","dir","actor1","actor2","actor3","time","metascore","rank","r1","r2","r3","r4","r5","r6","r7","r8","r9","r10")
